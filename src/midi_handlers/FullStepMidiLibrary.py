...
import numpy as np
from midi_handlers.FullStepMidiArrayBuilder import FullStepMidiArrayBuilder
import scipy.sparse

from midi_handlers.MusicLibrary import MusicLibraryFlat


class FullStepMidiArrayFlat(MusicLibraryFlat):

    def __init__(self, base_dir="", filenames=None, autoload=True):
        super().__init__(FullStepMidiArrayBuilder, base_dir, filenames, autoload)
...
def main():
    ...
    lib = FullStepMidiArrayFlat(os.path.join("midi", lib_name))
